...
func createVxlanLink(name string, suffix string, master string, vxlanId int) (netlink.Link, error) {
	...
	if bridgeCreated {
		...
		var bridgeRule = []string{"-o", vxlanBridgeName, "-j", "ACCEPT"}
		var iptable = iptables.GetIptable(iptables.IPv4)
		if !iptable.Exists(iptables.Filter, "FORWARD", bridgeRule...) {
...
func deleteVxlanLink(vxlanName string, vxlanBridgeName string) error {
	...
	// Delete forward rule to iptables
	var bridgeRule = []string{"-o", vxlanBridgeName, "-j", "ACCEPT"}
	var iptable = iptables.GetIptable(iptables.IPv4)
	if iptable.Exists(iptables.Filter, "FORWARD", bridgeRule...) {
		if err := iptable.ProgramRule(iptables.Filter, "FORWARD", iptables.Delete, bridgeRule); err != nil {
			return fmt.Errorf("failed to add delete rule of %q: %v", vxlanBridgeName, err)
		}
	}
